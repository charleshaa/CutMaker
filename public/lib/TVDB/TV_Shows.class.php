<?php
	...
	class TV_Shows extends TVDB {
		...
		public static function getBanners($showId) {
			$params = array('action' => 'get_banners', 'id' => $showId);
			$data = self::request($params);
			//print_r($data);
			if ($data) {
				$xml = @simplexml_load_string($data);
		...
		public static function getCast($showId) {
			$params = array('action' => 'get_cast', 'id' => $showId);
			$data = self::request($params);
			//print_r($data);
			if ($data) {
				$xml = @simplexml_load_string($data);
